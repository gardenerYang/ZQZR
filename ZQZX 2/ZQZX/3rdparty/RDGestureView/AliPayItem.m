...
- (CAShapeLayer *)outterLayer
{
    if (_outterLayer==nil) {
        ...
        _outterBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        _outterBtn.frame = CGRectMake( (self.frame.size.width-ITEMRADIUS_OUTTER)/2 , (self.frame.size.width-ITEMRADIUS_OUTTER)/2 , ITEMRADIUS_OUTTER , ITEMRADIUS_OUTTER);
        _outterBtn.backgroundColor =[UIColor redColor];
        _outterBtn.clipsToBounds  =  YES;
        _outterBtn.layer.cornerRadius = ITEMRADIUS_OUTTER/2;
        
        if (ITEMRADIUS_IMG) {
            [_outterBtn setImage:[UIImage imageNamed:ITEMRADIUS_IMG_NAME] forState:UIControlStateNormal];
        }
        _outterBtn.hidden = YES;
