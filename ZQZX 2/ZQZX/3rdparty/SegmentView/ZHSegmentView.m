...
-(void)setTitleArray:(NSArray *)titleArray{
    ...
    self.bottomLine.backgroundColor = _titleSelectColor;
...
-(void)btnIndexClick:(id)sender{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(ZHSegment:didSelectIndex:)]) {
        [self.delegate ZHSegment:self didSelectIndex:btn.tag];
    }
...
-(void)ZHDidScrollChangeTheTitleColorWithContentOfSet:(CGFloat)width{
    NSInteger leftIndex = width/Iphonewidth -1;
    if (leftIndex < 0) {
        leftIndex = 0;
    }
    ...
    //计算右边按钮的偏移比 相对比
    CGFloat rightScale  = width/Iphonewidth;
