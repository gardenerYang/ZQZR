...
- (void)setupLayoutWithDict:(NSDictionary *)dict andWidth:(CGFloat)width{
    ...
    if ([dict[@"type"] intValue] == SZQuestionOpenQuestion) {
        ...
        UILabel *lbl = ({
            lbl = [[UILabel alloc] initWithFrame:CGRectMake(self.configure.optionSideMargin + self.configure.buttonSize, 0, optionWidth, option_height)];
            lbl.numberOfLines = 0;
...
- (void)clickButton:(UIButton *)sender {
    ...
    if (self.type == SZQuestionSingleChoice) {
        NSArray * arr = self.contentDict[@"option"];
