...
@interface OrderdetailsViewController ()<MWPhotoBrowserDelegate>
@property(nonatomic,assign)BOOL isClick;
@property(nonatomic,strong)UIImageView *img;
@property(nonatomic,strong)NSMutableArray *dataListArr;
@property(nonatomic,strong)MyOrderDetailsModel *model;
...
-(void)requestList
{
    ...
    [HttpRequest myOrderDetailsOrderID:_ordID Requestsuccess:^(MyOrderDetailsModel * _Nonnull model, NSString * _Nonnull message) {
...
-(void)setupFootVie{
    ...
    [_cancelBtn addAction:^(UIButton *sender) {
        ...
        TipsVC.srcLbText = @"你确定要取消订单吗?";
        [TipsVC setTapCancelBtnblock:^{
            
        }];
        [TipsVC setTapConfirmBtnblock:^{
            [MBProgressHUD showActivityMessageInView:@"正在提交..."];
            [HttpRequest cancelOrderID:wf.ordID productID:@(wf.model.productId).stringValue Requestsuccess:^(HttpResponse * _Nonnull data, NSString * _Nonnull message) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        cell.timeView.srcLabel.text = _model.payTime;
        ...
        [cell.bgImgView sd_setBigImageWithString:_model.certificateUrl.count == 0 ? @"" : _model.certificateUrl[0]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (int i = 0; i < _model.certificateUrl.count; i++) {
         [photos addObject:[MWPhoto photoWithURL:[NSURL URLWithString: _model.certificateUrl[i]]]];
    }
