...
@property(nonatomic,strong)NSString *monthStr;//初步选择的月份数据
...
- (void)viewDidLoad {
    ...
    wf.dataStatusStr = @"oneMonth";
    wf.typeStr = @"";
    wf.statusStr = @"";
    wf.monthStr =  @"oneMonth";
_dataArr = @[@"待受理",@"待签约",@"已签约",@"签约作废",@"签约不成功",@"已取消",@"已退款",@"待赎回",@"已赎回",@"待还款",@"已还款",@"待签约"];
    _conditionFilterView = [QZConditionFilterView conditionFilterViewWithFilterBlock:^(BOOL isFilter, NSArray *dataSource1Ary, NSArray *dataSource2Ary, NSArray *dataSource3Ary) {
        ...
        if (isFilter) {
            //网络加载请求 存储请求参数
            wf.selectedDataSource1Ary = dataSource1Ary.count == 0 ?  @[wf.monthStr] : dataSource1Ary ;
            wf.selectedDataSource2Ary = dataSource2Ary ? @[@"全部"] : dataSource2Ary ;
            wf.selectedDataSource3Ary = dataSource3Ary ? @[@"全部"] : dataSource3Ary ;
            if (dataSource1Ary.count == 0) {
                wf.dataStatusStr = wf.monthStr;
            ...
            else
            {
                wf.dataStatusStr = [wf dataStatus:dataSource1Ary[0]];
                 wf.monthStr= [wf dataStatus:dataSource1Ary[0]];
            }
...
{
    __weak typeof(self) wf = self;
    [HttpRequest myOrderPageNum:_page dataStatus:_dataStatusStr type:_typeStr status:_statusStr Requestsuccess:^(MyOrderModel *_Nonnull model, NSString * _Nonnull message) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyOrderListModel *model = self.dataListArr[indexPath.row];
    ...
    cell.numLb.text = [self annualRate:model.rateYear];
    return cell;
...
-(NSString *)dataStatus:(NSString *)str{
    if ([str isEqualToString:@"一个月"]) {
        return @"oneMonth";
    }else if ([str isEqualToString:@"三个月"]){
        return @"threeMonth";
    }else if ([str isEqualToString:@"六个月"]){
        return @"halfYear";
    }else if ([str isEqualToString:@"一年"]){
        return @"oneYear";
    }
    else{
        return _monthStr;
